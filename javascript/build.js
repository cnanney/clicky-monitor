...
ClickyChrome.Build.basics = function () {
  ...
  var siteInfo = localStorage['clickychrome_currentSite'].split(','),
    linkURL =
      'http://getclicky.com/stats/home?site_id=' +
      siteInfo[0] +
      '&date=' +
      localStorage['clickychrome_currentDate'],
    spyURL =
      'http://getclicky.com/stats/spy?site_id=' +
      siteInfo[0] +
      '&date=' +
      localStorage['clickychrome_currentDate'],
    visitorsURL =
      'http://getclicky.com/stats/visitors?site_id=' +
      siteInfo[0] +
      '&date=' +
      localStorage['clickychrome_currentDate'],
    actionsURL =
      'http://getclicky.com/stats/visitors-actions?site_id=' +
      siteInfo[0] +
      '&date=' +
      localStorage['clickychrome_currentDate'],
    goalsURL =
      'http://getclicky.com/stats/goals?site_id=' +
      siteInfo[0] +
      '&date=' +
      localStorage['clickychrome_currentDate'],
    ...
    apiString =
      ...
      '&date=' +
      localStorage['clickychrome_currentDate'] +
      '&type=visitors-online,visitors,actions,actions-average,time-total-pretty,' +
    ...
    success: function (data) {
      if (data && data[0]) {
        ...
        if (data[0].error) {
          ...
          html =
            '<table class="basics_table" cellpadding="0" cellspacing="0">' +
            '<tr><td class="left visitors"><a class="inline_external external" href="' +
            visitorsURL +
            '">Visitors</a>'
          ...
          html +=
            ...
            info.visitors +
            '</td></tr>' +
            '<tr class="alt"><td class="left actions"><a class="inline_external external" href="' +
            actionsURL +
            '">Actions</td>' +
            '<td class="value">' +
            info.actions +
            ...
            info.averageActions +
            '</td></tr>' +
            '<tr class="alt"><td class="left time">Total time spent</td><td class="value">' +
            info.time +
            '</td></tr>' +
            '<tr><td class="left time_average">Average time per visit</td><td class="value">' +
            info.averageTime +
            '</td></tr>' +
            '<tr class="alt"><td class="left bounce">Bounce rate</td><td class="value">' +
            info.bounce +
            '%</td></tr>' +
            '<tr><td class="left goal"><a class="inline_external external" href="' +
            goalsURL +
            '">Goals</td>' +
            '<td class="value">' +
            info.goals +
            '</td></tr>' +
            '</table><p id="link_to_clicky"><a class="external" href="' +
            linkURL +
            '">' +
            linkText +
            '</a></p>'
...
ClickyChrome.Build.visitors = function () {
  ...
  var siteInfo = localStorage['clickychrome_currentSite'].split(','),
    ...
    apiString =
      ...
      '&sitekey=' +
      siteInfo[1] +
      '&type=visitors-list&output=json&limit=5&app=clickychrome'
    ...
    success: function (data) {
      if (data && data[0]) {
        ...
        if (data[0].error) {
          ...
          if (info.length == 0) {
            ...
            for (var i = 0, c = info.length; i < c; i++) {
              ...
              html +=
                '<div class="visitor' +
                odd +
                '"><div class="visitor_info"><span class="visitor_flag"><img src="' +
                info[i].flagImg +
                '" alt="' +
                info[i].geoLoc +
                '" /></span>' +
                ...
                '">' +
                displayName +
                '</a></span></div><div class="visitor_session">' +
                info[i].time +
                ' - ' +
                info[i].timeTotal +
                '<span class="' +
                actionClass +
                '">Actions: <a class="external" href="' +
                info[i].statsUrl +
                '">' +
                info[i].actions +
                '</a></span></div>' +
                '<div class="visitor_landed"><b>Landed:</b> <a class="external" href="' +
                info[i].contentUrl +
                '">' +
                info[i].landed +
                '</a></div>'
              ...
              if (info[i].referrerDomain !== false) {
                html +=
                  '<div class="visitor_from"><b>From:</b> <a class="external" href="' +
                  info[i].referrerUrl +
                  '">' +
                  info[i].referrerDomain +
                  '</a>'
          ...
          html +=
            '<p id="link_to_clicky"><a class="external" href="' +
            linkURL +
            '">' +
            linkText +
            '</a></p>'
...
ClickyChrome.Build.charts = function () {
  ...
  if (localStorage['clickychrome_currentChart'] != 'web-browsers') {
    ;(apiString =
      'http://api.getclicky.com/stats/api4?site_id=' +
      siteInfo[0] +
      '&sitekey=' +
      siteInfo[1] +
      '&type=' +
      localStorage['clickychrome_currentChart'] +
      '&date=previous-30-days&output=json&daily=1&app=clickychrome'),
      (linkUrl =
        'http://getclicky.com/stats/' +
        localStorage['clickychrome_currentChart'] +
        '?site_id=' +
        siteInfo[0]),
      (linkText = 'View ' + siteInfo[2] + ' on Clicky'),
        ...
        if (data && data[0]) {
          if (data[0].error) {
            ...
            if (data[0].dates[0].items.length > 0) {
              ...
              ClickyChrome.Functions.drawChart(
                tmpData.join(','),
                tmpLabels.join(','),
                localStorage['clickychrome_currentChart']
              )
  ...
  if (localStorage['clickychrome_currentChart'] == 'web-browsers') {
    ;(apiString =
      ...
      '&type=' +
      localStorage['clickychrome_currentChart'] +
      '&date=last-30-days&output=json&limit=11&app=clickychrome'),
      ...
      success: function (data) {
        if (data && data[0]) {
          if (data[0].error) {
            ...
            if (data[0].dates[0].items.length > 0) {
              var len = data[0].dates[0].items.length,
                i
              for (i = 0; i < len; i++) {
              ...
              if (len > 10) {
                tmpStatURLs[9] =
                  'http://getclicky.com/stats/visitors-browsers?site_id=' +
                  siteInfo[0] +
                  '&date=last-30-days'
                var pTotal = 0
                for (i = 0; i < 9; i++) {
              ...
              $('#content').html('<div id="chart_area"><div id="chart"></div></div>')
              ClickyChrome.Functions.drawPie(
                tmpData.slice(0, 10),
                tmpLabels.slice(0, 10),
                tmpStatURLs.slice(0, 10)
              )
              $('#chart_area').append(
                '<p id="link_to_clicky"><a class="external" href="' +
                  linkUrl +
