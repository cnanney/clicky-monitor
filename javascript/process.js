...
ClickyChrome.Process.visitors = function (data) {
  ...
  for (var i = 0, count = data.length; i < count; i++) {
    info[i] = {
      ...
      flagImg:
        typeof data[i].country_code == 'undefined'
      ...
      referrerDomain:
        typeof data[i].referrer_domain == 'undefined' ? false : data[i].referrer_domain,
      referrerUrl: typeof data[i].referrer_url == 'undefined' ? false : data[i].referrer_url,
      referrerSearch:
        typeof data[i].referrer_search == 'undefined' ? false : data[i].referrer_search,
    }
  }
  return info
}
...
ClickyChrome.Process.goals = function (data) {
  ...
  for (var i = 0; i < count; i++) {
    newIds[data[i].session_id] = {
      ...
      ip: data[i].ip_address,
      visitor:
        data[i].custom && data[i].custom.username ? data[i].custom.username : data[i].ip_address,
