...
ClickyChrome.Notifications.debug =
  chrome.extension.getBackgroundPage().ClickyChrome.Background.debug
...
ClickyChrome.Notifications.buildNotification = function () {
  ...
  html +=
    '<div id="notification_header"><span id="scrollLeft"><a href="#" id="scrollLeftButton"><img src="/images/icon_left.png" alt="Scroll left" /></a></span>' +
    title +
    '<span id="scrollRight"><a href="#" id="scrollRightButton"><img src="/images/icon_right.png" alt="Scroll right" /></a></span></div>' +
  ...
  for (var id in goals) {
    if (goals.hasOwnProperty(id) && id != 'id') {
      ...
      ipLink =
        'http://getclicky.com/stats/visitors?site_id=' + siteInfo[0] + '&ip_address=' + goals[id].ip
      flagUrl =
        goals[id].cc == 'none'
          ? '/images/icon_world.png'
          : 'http://static.getclicky.com/media/flags/' + goals[id].cc + '.gif'
      value = goals[id].value == '' ? '' : '$' + goals[id].value

      html +=
        '<div class="notification_body"><div class="goal_value">' +
        value +
        '</div>' +
        '<div class="goal_name"><img src="/images/icon_goal.gif" class="goal_icon" alt="Goals Completed" />' +
        goals[id].goals +
        '</div><div class="goal_user"><span class="' +
        displayClass +
        ...
        goals[id].visitor +
        '</a></span></div>' +
        '<div class="goal_geo"><img src="' +
        flagUrl +
        '" class="goal_flag" alt="Geolocation" />' +
        goals[id].geo +
        '</div><p class="session_link"><a class="external" target="_blank" href="' +
        goals[id].url +
        '">' +
        goals[id].time +
        '</a></p></div>'
  ...
  if (this.debug)
    chrome.extension
      .getBackgroundPage()
      .ClickyChrome.Background.log('Notification width: ' + $('#notification_container').width())
  this.checkScroll()
}
...
ClickyChrome.Notifications.checkScroll = function () {
  if (this.vars.slide < this.vars.count) $('#scrollRightButton').show()
  else $('#scrollRightButton').hide()
  if (this.vars.slide > 1) $('#scrollLeftButton').show()
  else $('#scrollLeftButton').hide()
}
...
ClickyChrome.Notifications.scrollRight = function () {
  if (this.vars.slide < this.vars.count) {
    $('#notification_container').animate(
      ...
      300,
      function () {
        if (ClickyChrome.Notifications.debug)
          chrome.extension.getBackgroundPage().ClickyChrome.Background.log('Scrolled right')
      }
    )
    this.vars.slide++
  }
...
ClickyChrome.Notifications.scrollLeft = function () {
  if (this.vars.slide > 1) {
    $('#notification_container').animate(
      ...
      function () {
        if (ClickyChrome.Notifications.debug)
          chrome.extension.getBackgroundPage().ClickyChrome.Background.log('Scrolled left')
      }
    )
    this.vars.slide--
  }
