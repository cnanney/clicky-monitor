...
$(function () {
  ...
  $('.add_site').click(function () {
    var string =
      '<tr><td><div class="input_name"><input class="input_name" name="name[]" /></div></td>' +
    ...
    if (num == 0) {
      ...
      $('input.input_url').each(function () {
        var value = $(this)
          .val()
          .replace(/^\s+|\s+$/g, '')
        value = value.replace(/(^[a-z][a-z0-9+\-.]*:\/\/)|(\/$)/gi, '')
        $(this).val(value)
        if (value.indexOf(',') != -1) {
          invalid_name = 1
        }
  ...
  $('#import_form').submit(function () {
    ...
    var username = escape($('#username').val()),
      ...
      apiString =
        'https://api.getclicky.com/api/account/sites?username=' +
        username +
        '&password=' +
        password +
        '&output=json&app=clickychrome'
...
ClickyChrome.Options.init = function () {
  ...
  $('.color_input').each(function () {
    if ($(this).val() == localStorage['clickychrome_badgeColor']) $(this).attr('checked', 'checked')
  })
  $('.spy_type').each(function () {
    if ($(this).val() == localStorage['clickychrome_spyType']) $(this).attr('checked', 'checked')
  })
...
ClickyChrome.Options.saveData = function (data) {
  ...
  $('#save_feedback')
    .show()
    .text('Options saved. You can close this tab.')
    .delay(6000)
    .fadeOut(1000)
...
ClickyChrome.Options.buildSiteTable = function () {
  ...
  for (var i = 0, c = this.vars.nameArray.length; i < c; i++) {
    var string =
      '<tr><td><div class="input_name"><img title="Drag to re-order" class="grip" src="/images/grippy.png" />' +
      this.vars.nameArray[i] +
      '</div><div class="input_name off"><input class="input_name" name="name[]" value="' +
      this.vars.nameArray[i] +
      '" /></div></td><td><div class="input_url">' +
      this.vars.urlArray[i] +
      '</div><div class="input_url off">' +
      '<input class="input_url" name="url[]" value="' +
      this.vars.urlArray[i] +
      '" /></div></td><td><div class="input_id">' +
      this.vars.idArray[i] +
      '</div><div class="input_id off"><input class="input_id" name="id[]" value="' +
      this.vars.idArray[i] +
      '" /></div></td><td><div class="input_key">' +
      this.vars.keyArray[i] +
      '</div><div class="input_key off"><input class="input_key" name="key[]" value="' +
      this.vars.keyArray[i] +
...
ClickyChrome.Options.checkSites = function () {
  ...
  if (num == 0) {
    var string =
      '<tr id="reminder"><td colspan="5">You must add at least one site from your Clicky account to use this extension.</td></tr>'
    $('tbody').append(string)
  } else {
    $('#reminder').remove()
  }
...
ClickyChrome.Options.resetCurrent = function () {
  localStorage['clickychrome_currentSite'] =
    this.vars.idArray[0] + ',' + this.vars.keyArray[0] + ',' + this.vars.nameArray[0]
}
